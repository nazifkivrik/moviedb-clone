<script setup>
  import { ref, inject, onBeforeMount, reactive, watch, computed } from 'vue'
  ...
  function setSelectedCompany(name) {
    if (!discoverOptions.Networks.includes(name)) {
      discoverOptions.Networks.push(name)
    }
  }
          ...
          <li class="companies">
            ...
            <ul class="selectedCompanies">
              <li v-for="company in discoverOptions.Networks" :key="company">
                ...
                <span
                  @click="
                    discoverOptions.Networks.splice(
                      discoverOptions.Networks.indexOf(company.name),
                      1
                    )
                  "
            ...
            <input type="text" placeholder="Filter by networks..." v-model="companySearchString" />
            <ul
              ref="keywordsContainer"
              @scroll="getCompanyPagesLazyDebounced($event)"
              :class="[searchedCompanies.length !== 0 ? 'companySearch' : 'passive']"
              v-click-outside="(e) => closeList(e, searchedCompanies)"
              id="keywordsCont"
              v-if="companySearchString">
              <li
                v-for="company in searchedCompanies"
                :key="company.id"
                @click="setSelectedCompany(company)">
                <img
                  :src="store.imageURL('w45', company.logo_path)"
                  alt=""
