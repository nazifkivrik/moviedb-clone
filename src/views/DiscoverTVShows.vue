<script setup>
  import discoverTVShowsOptions from '../components/discoverTVShowsOptions.vue'
  ...
  async function discover(discoverOptions) {
    ...
    if (discoverOptions.Networks.length > 0) {
      let arr = []
...
<template>
  <main>
    <div class="options"><discoverTVShowsOptions @discover="discover" /></div>
    <div class="container">
      <ul>
        <li v-for="item in discovered" :key="item">
          <app-link :to="`/tv/${item.id}`"
            ><img
          ...
          <span class="info">
            <app-link :to="`/movie/${item.id}`">{{ item.name }}</app-link>
